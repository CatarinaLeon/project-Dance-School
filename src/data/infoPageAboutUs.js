import Img_1_mob_1x from '../images/Dance-Styles/DS-IMG-1-mob-1x.webp';
import Img_1_mob_2x from '../images/Dance-Styles/DS-IMG-1-mob-2x.webp';
import Img_1_tablet_1x from '../images/Dance-Styles/DS-IMG-1-tablet-1x.webp';
import Img_1_tablet_2x from '../images/Dance-Styles/DS-IMG-1-tablet-2x.webp';
import Img_1_desktop_1x from '../images/Dance-Styles/DS-IMG-1-desktop-1x.webp';
import Img_1_desktop_2x from '../images/Dance-Styles/DS-IMG-1-desktop-2x.webp';

import Img_2_mob_1x from '../images/Dance-Styles/DS-IMG-4-mob-1x.webp';
import Img_2_mob_2x from '../images/Dance-Styles/DS-IMG-4-mob-2x.webp';
import Img_2_tablet_1x from '../images/Dance-Styles/DS-IMG-4-tablet-1x.webp';
import Img_2_tablet_2x from '../images/Dance-Styles/DS-IMG-4-tablet-2x.webp';
import Img_2_desktop_1x from '../images/Dance-Styles/DS-IMG-4-desktop-1x.webp';
import Img_2_desktop_2x from '../images/Dance-Styles/DS-IMG-4-desktop-2x.webp';

import Img_6_mob_1x from '../images/Dance-Styles/DS-IMG-6-mob-1x.webp';
import Img_6_mob_2x from '../images/Dance-Styles/DS-IMG-6-mob-2x.webp';
import Img_6_tablet_1x from '../images/Dance-Styles/DS-IMG-6-tablet-1x.webp';
import Img_6_tablet_2x from '../images/Dance-Styles/DS-IMG-6-tablet-2x.webp';
import Img_6_desktop_1x from '../images/Dance-Styles/DS-IMG-6-desktop-1x.webp';
import Img_6_desktop_2x from '../images/Dance-Styles/DS-IMG-6-desktop-2x.webp';

import Img_7_mob_1x from '../images/Dance-Styles/DS-IMG-7-mob-1x.webp';
import Img_7_mob_2x from '../images/Dance-Styles/DS-IMG-7-mob-2x.webp';
import Img_7_tablet_1x from '../images/Dance-Styles/DS-IMG-7-tablet-1x.webp';
import Img_7_tablet_2x from '../images/Dance-Styles/DS-IMG-7-tablet-2x.webp';
import Img_7_desktop_1x from '../images/Dance-Styles/DS-IMG-7-desktop-1x.webp';
import Img_7_desktop_2x from '../images/Dance-Styles/DS-IMG-7-desktop-2x.webp';

export const pageAboutUsConfig = [
  {
    img_mob_1x: Img_1_mob_1x,
    img_mob_2x: Img_1_mob_2x,
    img_tablet_1x: Img_1_tablet_1x,
    img_tablet_2x: Img_1_tablet_2x,
    img_desktop_1x: Img_1_desktop_1x,
    img_desktop_2x: Img_1_desktop_2x,
    altImg: 'Social dancing',
    title: 'Group classes',
    text: 'If you like to practice in a group or want to find new friends and have fun, look at our group classes. Try Swing, Tango, Salsa, and more! Everyone is welcome—just look for the beginner, advanced, and all labels in the schedule to make sure you feel comfortable during the class and get joy, not the achy feet. To start, choose your dance style—Social Dance, International System, or American System. We also have a program for Kids',
    url: '/',
  },
  {
    img_mob_1x: Img_2_mob_1x,
    img_mob_2x: Img_2_mob_2x,
    img_tablet_1x: Img_2_tablet_1x,
    img_tablet_2x: Img_2_tablet_2x,
    img_desktop_1x: Img_2_desktop_1x,
    img_desktop_2x: Img_2_desktop_2x,
    altImg: 'private lessons',
    title: 'Private lessons ',
    text: 'Private classes offer the most in-depth and individual approach to your learning. Your PERSONAL TRAINER will help you set and achieve goals and will develop a personalized plan for each of your classes. Make your UNIQUE WAY through the ballroom! Getting married? We`ll help you make your WEDDING dance something to remember.',
    url: '/',
  },
  {
    img_mob_1x: Img_6_mob_1x,
    img_mob_2x: Img_6_mob_2x,
    img_tablet_1x: Img_6_tablet_1x,
    img_tablet_2x: Img_6_tablet_2x,
    img_desktop_1x: Img_6_desktop_1x,
    img_desktop_2x: Img_6_desktop_2x,
    altImg: 'Parties',
    title: 'Special events ',
    text: 'Workshops, themed parties, dance nights, and other events are an important part of our school`s life. Bring everything you`ve learned to the dance floor and meet other students. We have the most welcoming atmosphere—at dance events, even the shiest of our students let themselves go.',
    url: '/',
  },
  {
    img_mob_1x: Img_7_mob_1x,
    img_mob_2x: Img_7_mob_2x,
    img_tablet_1x: Img_7_tablet_1x,
    img_tablet_2x: Img_7_tablet_2x,
    img_desktop_1x: Img_7_desktop_1x,
    img_desktop_2x: Img_7_desktop_2x,
    altImg: 'Advanced ballroom',
    title: 'On-line classes',
    text: 'Keep dancing and getting joy from every movement even when you cannot attend an offline class. All classes are now held online. Check out the schedule and enroll!',
    url: '/',
  },
];
